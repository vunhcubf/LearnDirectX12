...
Texture::Texture(D3D12_SHADER_RESOURCE_VIEW_DESC* descriptor, CD3DX12_RESOURCE_DESC textureDesc,Graphics* graphics) {
	THROW_IF_ERROR(graphics->pID3DDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,&textureDesc,D3D12_RESOURCE_STATE_COMMON,nullptr,IID_PPV_ARGS(&this->TextueResource)));

	this->TextureViewIndex = graphics->AddViewOnSRVHeap(descriptor, this->TextueResource.Get());
}
...
Texture::Texture(int width,int height,int arraysize,int miplevel,DXGI_FORMAT Format, D3D12_SRV_DIMENSION dimension, D3D12_TEX2D_SRV type, Graphics* graphics)
{
	...
	TextureViewIndex = graphics->AddViewOnSRVHeap(&descriptor, this->TextueResource.Get());
}
...
Texture::Texture(std::wstring filepath, Graphics* graphics)
{
	...
	TextureViewIndex = graphics->AddViewOnSRVHeap(&srvDesc, TextueResource.Get());
}
